...
it.each(['assignments','deadlines','upcoming','todo'] as const)('renders the %s Python contract without raw identifiers', key => {
  const result = {status:'success',[key]:fixture.expected[key]} as Extract<Result,{status:'success'}>;
  const html = renderToStaticMarkup(<ResultList result={result} onCourse={() => {}}/>);
...
it('keeps completed/non-candidate deadline rows visible', () => {
  const html = renderToStaticMarkup(<ResultList result={{status:'success',deadlines:fixture.edge.expected.deadlines}} onCourse={() => {}}/>);
  expect(html).toContain('채점됨');
  expect(html).toContain('남은 과제 후보 아님');
});
it('shows empty result and never renders API markup as HTML', () => {
  expect(renderToStaticMarkup(<ResultList result={{status:'success',todo:[]}} onCourse={() => {}}/>)).toContain('조회된 항목이 없습니다');
  const result = {status:'success',todo:[{title:'<script>bad()</script>',due_at:'',course:'',type:'',ignore:false}]} as const;
  const html = renderToStaticMarkup(<ResultList result={{...result,todo:[...result.todo]}} onCourse={() => {}}/>);
