...
function label(value: unknown, ...rows: Row[]): string {
  return redactText(text(value), rows.flatMap(privateIds));
}
...
export function isoTime(value: string): number {
  ...
  if (!match) return NaN;
  const [, y, m, d, h = "00", min = "00", sec = "00", fraction = "", zone] =
    match;
  ...
  return (
    date.getTime() - offset + Number(fraction.padEnd(6, "0").slice(3)) / 1000
  );
}
export function remainingCandidate(
  due: string,
  locked: boolean,
  submitted: string,
  workflow: string,
  ...
  return rows(raw).map((row) => {
    ...
    if (
      ...
      (typeof row.points_possible !== "number" ||
        !Number.isFinite(row.points_possible))
    )
      throw new Error("INVALID_RESPONSE");
    if (
      row.submission_types != null &&
      (!Array.isArray(row.submission_types) || row.submission_types.length > 50)
    )
      throw new Error("INVALID_RESPONSE");
    return {
...
export function projectDeadlines(assignments: Assignment[]): Deadline[] {
  return assignments.map(({ title, due_at, remaining_candidate }) => ({
    title,
    due_at,
    remaining_candidate,
  }));
