...
export interface TranscriptItem {index:number;time:string;text:string}
...
export function formatTime(raw: string): string {
  ...
  if (!match || Number(match[2]) > 59 || Number(match[3]) > 59) throw new Error('INVALID_TIME');
  const hours = Number(match[1] ?? 0);
  return `${hours ? `${String(hours).padStart(2,'0')}:` : ''}${match[2]}:${match[3]}`;
}
export function cleanText(raw: string): string {
  return raw.replace(/<br\s*\/?\s*>/gi,' ').replace(/<[^>]*>/g,'')
    .replace(/&(?:amp|lt|gt|quot|apos|nbsp|#\d+|#x[0-9a-f]+);/gi, entity => {
      const code = entity.slice(1,-1).toLowerCase();
      if (code.startsWith('#')) {
        const value = code.startsWith('#x') ? parseInt(code.slice(2),16) : Number(code.slice(1));
        return value > 0 && value <= 0x10ffff ? String.fromCodePoint(value) : entity;
      }
      return ({amp:'&',lt:'<',gt:'>',quot:'"',apos:"'",nbsp:' '} as Record<string,string>)[code] ?? entity;
    }).replace(/\s+/g,' ').trim();
}
export function normalizeItems(input: unknown): TranscriptItem[] {
  ...
  return input.map((item: unknown,index) => {
    ...
    if (typeof row.time !== 'string' || typeof row.text !== 'string') throw new Error('INVALID_CAPTION');
...
export function parseVtt(raw: string): TranscriptItem[] {
  ...
  for (const block of content.split(/\n[ \t]*\n/)) {
    ...
    const match = /^\s*((?:\d{2,}:)?\d{2}:\d{2}\.\d{3})\s+-->\s+((?:\d{2,}:)?\d{2}:\d{2}\.\d{3})(?:\s+.*)?$/.exec(lines[at]!);
...
export function transcriptText(value: Transcript): string {
  return value.items.map(item => `${item.time ? item.time+' ' : ''}${item.text}`).join('\n') + '\n';
}
...
export function parseMediaScript(raw: string): TranscriptItem[] {
  ...
  if (!text || /<(?:!doctype|html|script)\b/i.test(text)) throw new Error('INVALID_SCRIPT');
  if (/^WEBVTT(?:\s|$)/.test(text)) return parseVtt(text);
  const items: {time:string;text:string}[] = [];
  let current: {time:string;text:string} | undefined;
  for (const line of text.split('\n')) {
    const range = /^\s*\[((?:\d{2,}:)?\d{2}:\d{2}(?:[.,]\d{1,3})?)\s*[~–-]\s*((?:\d{2,}:)?\d{2}:\d{2}(?:[.,]\d{1,3})?)\]\s*$/.exec(line);
    if (range) {
