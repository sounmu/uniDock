...
export const errors = ['LOGIN_REQUIRED', 'OPEN_LMS', 'RELOAD_TAB', 'FORBIDDEN', 'NETWORK', 'TIMEOUT', 'INVALID_RESPONSE', 'POLICY', 'LIMIT', 'COURSE_NOT_FOUND', 'COURSE_AMBIGUOUS', 'BUSY', 'STALE_SELECTION', 'TAB_OPEN_FAILED'] as const;
export type ErrorCode = typeof errors[number];
export type ListRequest = { version: 1; type: 'COURSES_LIST' | 'TODO_LIST' }
  | { version: 1; type: 'ASSIGNMENTS_LIST' | 'DEADLINES_LIST' | 'RECORDINGS_LIST'; course: string }
  | { version: 1; type: 'UPCOMING_LIST'; start_date?: string; end_date?: string };
export type Request = ListRequest | {version:1;type:'RECORDING_OPEN';handle:string};
export function validHandle(value: unknown): value is string { return typeof value === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(value); }
export type Result = {status:'success'; recordings:Recording[]} | {status:'success';opened:true}
...
export function isRequest(value: unknown): value is Request {
  ...
  if (row.type === 'RECORDING_OPEN') return keys.length === 3 && validHandle(row.handle);
  if (row.type === 'ASSIGNMENTS_LIST' || row.type === 'DEADLINES_LIST' || row.type === 'RECORDINGS_LIST') return keys.length === 3 && typeof row.course === 'string' && row.course.trim().length > 0 && row.course.length <= 2000 && redactText(row.course) === row.course;
...
const fields = {
  recordings: { module:'text', title:'text', type:'externalTool', lmsHandle:'handle', launchHandle:'optionalHandle' },
...
const resultKey = { COURSES_LIST: 'courses', ASSIGNMENTS_LIST: 'assignments', DEADLINES_LIST: 'deadlines', UPCOMING_LIST: 'upcoming', TODO_LIST: 'todo', RECORDINGS_LIST:'recordings', RECORDING_OPEN:'opened' } as const;
export function parseResult(value: unknown, expected?: Request): Result {
  try {
    if (value && typeof value === 'object') {
      ...
      if (row.status === 'success' && row.opened === true && Object.keys(row).length === 2 && (!expected || expected.type === 'RECORDING_OPEN')) return {status:'success',opened:true};
      ...
      const items = raw.map((item: unknown) => {
        ...
        return Object.fromEntries(Object.entries(schema).map(([field, type]) => {
          ...
          if (type === 'externalTool' && child === 'ExternalTool') return [field, child];
          if ((type === 'handle' || type === 'optionalHandle') && (validHandle(child) || (type === 'optionalHandle' && child === ''))) return [field, child];
