...
export function nextPage(header: string | null, origin: string, path = '/api/v1/courses'): string | null {
  ...
  for (const part of header.split(/,(?=\s*<)/)) {
    ...
    if ((rel[1] ?? rel[2] ?? '').split(/\s+/).includes('next')) next.push(readUrl(match[1]!, origin, path).href);
  }
