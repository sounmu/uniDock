...
export function redactText(text: string, privateValues: string[] = []): string {
  let safe = text;
  for (const value of privateValues) if (value) safe = safe.split(value).join(hidden);
  return safe
    .replace(/https?:\/\/[^\s<>"']+/gi, hidden)
    .replace(/[\w.%+-]+@[\w.-]+\.[a-z]{2,}/gi, hidden)
    .replace(/(?:authorization|bearer|cookie|token|password|pwd|session|saml\w*|oauth\w*|course[_ -]?id|KU_LMS_ID)\s*[:=]?\s*[^\n]+/gi, hidden)
