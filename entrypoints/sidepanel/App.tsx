...
const tabs = [ ['COURSES_LIST', '내 과목'], ['ASSIGNMENTS_LIST', '과제'], ['DEADLINES_LIST', '마감일'], ['UPCOMING_LIST', 'Upcoming'], ['TODO_LIST', 'Todo'] ] as const;
export function App() {
  const [view, setView] = useState<Request['type']>('COURSES_LIST');
  ...
  const needsCourse = view === 'ASSIGNMENTS_LIST' || view === 'DEADLINES_LIST';
  ...
  return <main>
    ...
    <section aria-live="polite" aria-busy={state.status === 'loading'}>
      ...
      {state.status === 'success' && <ResultList result={state} onCourse={name => {setCourse(name);setView('ASSIGNMENTS_LIST');void load({version:1,type:'ASSIGNMENTS_LIST',course:name});}}/>}
    </section>
