...
export default defineContentScript({
  ...
  main() {
    ...
    async function open(handle: string): Promise<Result> {
      ...
      try {
        const result: unknown = await chrome.runtime.sendMessage({version:1,type:'OPEN_LMS_TARGET',url});
        return parseResult(result,{version:1,type:'RECORDING_OPEN',handle});
      } catch { return {status:'error',code:'TAB_OPEN_FAILED'}; }
    }
    ...
    chrome.runtime.onMessage.addListener((message: unknown, sender, respond) => {
      ...
      if (!pending) {
        ...
        const result = message.type === 'RECORDING_OPEN' ? open(message.handle) : message.type === 'COURSES_LIST' ? listCourses(location.origin) : message.type === 'RECORDINGS_LIST' ? listQuery(location.origin,message,fetch,Date.now(),catalog) : listQuery(location.origin,message);
        pending = {key,result:result.finally(() => { pending = undefined; })};
      }
